...
angular.module("fac",[]).service('WEB_CONFIG', function () {
	var data={cart:localStorage.getObjet('cart')||[],item:{}};
	return data;
}).directive('randomClass', function () {
	return {restrict: 'A',replace: true,scope: {randomClass:'='},link: function(scope, element,attr){
	$(element).addClass(attr.prefex+scope.randomClass[getRandomInt(0,(scope.randomClass.length-1))]);
	}
}
}).factory('HttpInterceptor', ['$rootScope', '$q', function ($rootScope, $q) {
			return {
				'request': function (config) {
					config.headers['X-Requested-With'] = 'XMLHttpRequest';config.headers['X-Csrf-Token'] =IsMobile;
					return config || $q.when(config);
				},
				'requestError': function (rejection) {$q.reject(rejection);},
				'response': function (response) {
					if(!DEBUG && response.config.headers.name=="decrypt"){try{var rs=(b64_to_utf8(response.data).substr(14));
					...
					if(response.data.test==false){
						$.notify(response.data.errors||'Error',{'type':'danger',placement: {from:"bottom",align:"right"},animate: {enter: 'animated fadeInUp ',exit: 'animated fadeOutDown'}});
					}
	...
	]).filter('unsafe', function ($sce) {return function (text) {if (typeof text != "undefined") {return $sce.trustAsHtml(text.replaceAll("\n", "<br>"))}}}).directive("rating",function($http){
    return{
        scope:{item:"@"},restrict: 'A',
        link:function($scope, $element, $attrs){
			$scope.first=true;$element=$($element);$element.data('rate',$attrs.rate).rating();$element.on("change",function(event){
			if($scope.first){
				$http.post(API+"fn?action=rateP",{id:$scope.item,rate:$element.val()}).then(function (e){if(e.data.test){$scope.first=false;$.fn.toast({ id: 'successs', content: 'Produits notée', type: 'success', delay: 8000});
				$element.val(e.data.rate).data('rate',e.data.rate).rating("refresh")
				}},function(){});
			}else{$.fn.toast({ id: 'successs', content: 'déjà notée', type: 'error', delay: 8000});$element.rating("refresh");}});
        }
    }
}).directive("cart",function($http,WEB_CONFIG){
    return{
        scope:{cart:"=?"},restrict: 'A',
        link:function($scope, $this, $attrs){$scope.DATA_CONFIG=WEB_CONFIG;
		$this.on("click",function(event){
			$product = $this.closest('.product');
			$cart=$scope.DATA_CONFIG.cart;$this.addClass('load-more-overlay loading');
			if($cart.filter((e)=>e.id==$scope.cart.id).length>0){
				$cart[$cart.findIndex((e)=>e.id==$scope.cart.id)].qn++;
			}else{
				$cart.push({url:$scope.cart.url,id:$scope.cart.id,img:$scope.cart.src,nom:$scope.cart.nom,prix:parseFloat($scope.cart.nprix),qn:parseFloat($("#singleQn").val()||1)});
			}
			$this.attr("disabled","disabled");
			$(".product-sticky-content").hide();
			setTimeout(function () {
                $this.removeClass('load-more-overlay loading');
                Trabna.Minipopup.open({
				productClass: ' product-cart',
				name: $scope.cart.nom,
				nameLink: $product.find('.product-name > a, .product-title > a').attr('href'),
				imageSrc: $product.find('.product-media img, .product-image:first-child img').attr('src'),
				imageLink: $product.find('.product-name > a').attr('href'),
				message:MinipopupHTML.title,
				actionTemplate: MinipopupHTML.msg
                });
            }, 500);
			$scope.$apply(function(){
				$scope.DATA_CONFIG.cart=$cart;
			})
			localStorage.setObjet('cart',$cart);
			});
        }
    }
}).directive("setdata",function($http){
    return{
        scope:{"setdata":"=?"},restrict: 'A',
        link:function($scope, $element, $attrs){
			var $cmpx=localStorage.getObjet('cmp')||[],$wish=localStorage.getObjet('wish')||[];
			if($wish.findIndex((e)=>e.id==$scope.setdata.id)>=0){
				$element.find('.btn-wishlist').toggleClass('w-icon-heart').toggleClass('w-icon-heart-full added');
			}
			if($cmpx.findIndex((e)=>e.id==$scope.setdata.id)>=0){
				$element.find('.btn-compare').toggleClass('w-icon-compare').addClass('w-icon-check-solid');
			}
		 $element.data($scope.setdata);
        }
    }
}).directive("countdown",function($http){
    return{
        scope:{},restrict: 'A',
        link:function($scope, $element, $attrs){
			if($attrs.until!="" && $.fn.countdown){
			$element.countdown({
                    until: new Date($attrs.until),
                    format: $attrs.format,
                    padZeroes: true,
                    compact: false,
                    compactLabels: [' m', ' w', ' days, '],
                    timeSeparator: ' : ',
					labels:$attrs.labelsShort.split(',')
                });
...
}).directive('product', function () {return {restrict: 'AE',replace:true,templateUrl:'product.html'};
}).filter('totalr', function() {
    return function(input) {
        var i = input instanceof Array ? input.length : 0;
            var total = 0;
            while (i--) {
                if (!isNaN(parseFloat(input[i]['qn']))) {
                    total += parseFloat(input[i]['qn'])*parseFloat(input[i]['prix']);
                }
            }
            return total;
    }
    ;
}).filter('totalx', function() {
    ...
    return{
        scope:{repeatDone:'&'},restrict: 'A',
        link:function($scope, $element, $attrs){if($scope.$parent.$last){$scope.repeatDone();}}
    }
}).filter('shunks', function() {
    return function chunk(items, chunk_size) {
      ...
      return chunks;
		...
		var link = function ($scope, $element, attrs) {
			$scope.loaded=true;console.log("ini scrol")
			$(window).on('scroll', function() {
				if($(this).scrollTop()>=($element.offset().top-400) && $scope.loaded){
					$scope.loaded=false;
					$scope.onScroll({id:$scope.idx,gt:attrs.id});
				}
			})
		};
		return {
	...
	}]).directive("sectionpub",function ($http,$filter,$window) {
		var previousScroll = 0;
		var link = function ($scope, $element, attrs) {
			$scope.loaded=true;$scope.DATA=$filter('shunks')([1,2,3,4,5,6,7,8],2);$scope.loadinng=true;
			$scope.repeatDone1=(e)=>{if($element[0].swiper!==undefined){$element[0].swiper.update()};}
			$(window).on('scroll',function() {
				if($(this).scrollTop()>=($element.offset().top-400) && $scope.loaded){
					$scope.loaded=false;
					$scope.loadinng=true;
					$http.post($scope.url,{id:$scope.idx},{headers:{name:'decrypt'}}).then(function (e){
					if(e.data.test){
						$scope.DATA=$filter('shunks')(e.data.d,2);
					};$scope.loadinng=false;
					},function(){$scope.DATA=[];$scope.loadinng=false;});
				}
			})
		};
		return {
			...
			link: link,
				template:`<div  class="swiper-container swiper-theme" data-swiper-options="{
					'spaceBetween': 5,
					'slidesPerView': 2.2,
					'breakpoints': {
					...
					<div class="swiper-wrapper  card is-loading  row cols-xl-4 cols-lg-3 cols-2">
					   
						<div class="swiper-slide product-col"  ng-repeat="y in DATA track by $index" >
						<div class="product-wrapper" ng-repeat="x in y" ng-if="loadinng"><div class="product"><div class="image"></div><div class="content"><h2></h2><h2 class="title2"></h2><h2 class="title2" style="width: 44%;"></h2><h2 class="title2"></h2></div></div>
                                    </div>
                            <div repeat-done="repeatDone1('#swtokp')" ng-repeat="x in y" class="product-wrapper" ng-if="!loadinng">
                                            <product />
                             </div>
	...
	}).directive("sectionpubhor",function ($http,$filter,$window) {
		var previousScroll = 0;
		var link = function ($scope, $element, attrs) {
			$scope.loaded=true;$scope.DATA=$filter('shunks')([1,2,3,4,5,6,7,8],2);$scope.loadinng=true;
			$scope.repeatDone1=(e)=>{if($element[0].swiper!==undefined){$element[0].swiper.update()};}
		...
		return {
			...
			restrict: "E",replace:true,
			link: link,
				template:`<div class="swiper-container swiper-theme mt-4" data-swiper-options="{
                            'spaceBetween': 5,
                            'slidesPerView': 2.2,
                            'breakpoints': {
                                '576': {
                            ...
                            <div class="swiper-wrapper row cols-md-4 cols-sm-3 cols-2">
                                <div class="swiper-slide card is-loading "  ng-repeat="x in DATA track by $index" repeat-done="repeatDone1()" >
								<div class="product-wrapper">
								
						<div ng-if="loadinng"  class="product"><div class="image"></div><div class="content"><h2></h2><h2 class="title2"></h2><h2 class="title2" style="width: 44%;"></h2><h2 class="title2"></h2></div></div>
						
                            <div  ng-if="!loadinng"><product /></div>
							</div>
						</div>
                            </div>
                        </div>`
		};
	}).directive("loadInfo",function($http,WEB_CONFIG){
    return{
        scope:{},restrict: 'A',
        link:function($scope, $element, $attrs){
			$element.on("click",()=>{
				WEB_CONFIG.popup($attrs.loadInfo);
			})
		}
    }
})
